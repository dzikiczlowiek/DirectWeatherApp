namespace DirectWeather.UnitTests.OpenWeatherMap.WeatherControllerTests
{
    ...
    public class GetWeatherTests : TestsBase, IClassFixture<GetWeatherFixture>
    {
        ...
        {
            getWeatherFixture.Accept(A.WeatherRequest().ForCity("Warsaw").InCountry("Poland")).Execute().AssertThat
        ...
        public void ensure_that_query_result_was_utilized_in_response_builder_mapping_method()
        {
            getWeatherFixture.Accept(A.WeatherRequest().ForCity("Warsaw").InCountry("Poland"))
                .AndTheResultForTheQueryIs(A.SuccessWeatherQueryResult()).Execute().AssertThat
